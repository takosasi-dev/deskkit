...
| モジュール | できること | 最初の動き |
|---|---|---|
| **ModeShift** | 「ゲーム」「勉強」などのモードを作り、電源プラン・音量(全体/アプリ別)・アプリの起動/終了・フォルダ/URL・ウィンドウ配置の呼び出しを1操作で実行。電源と音量は元に戻せる | 新しいモードは初回にプレビューで確認してから実行 |
| **DropSort** | ダウンロードが**完了した**ファイルだけをルールで振り分け。開かない・実行しない・MOTW(インターネット由来の印)を必ず保つ。元に戻せる | ルールは試運転(予定を表示するだけ)から |
| **LayoutKeep** | モニタ構成ごとにウィンドウ配置を保存し、抜き差し・スリープ復帰で崩れたら戻す。区別できないウィンドウは動かさない | 試運転(計画を表示するだけ)から |
| **ClipShelf** | コピーしたテキストを DPAPI で暗号化して記録し、`Ctrl+Alt+V` のパレットで検索・呼び出し。定型文・書式なし貼り付け | 観察モード(記録しない)から |
